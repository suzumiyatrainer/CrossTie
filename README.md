...
### 3. サーバー互換性
- **クラッシュ回避**: クライアント専用の描画コードを適切に分離し、サーバーのmodsフォルダに入れてもクラッシュしない設計にアップデートしました。
- **導入メリット**: 基本的にはクライアント導入で効果を発揮しますが、サーバーへの導入も安全です。ただし期待値は低めです。(安全最優先のため、サーバー側の最適化は最小限に留めています。)
